...
export default {
    ...
    methods: {
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    axios.get('http://172.20.10.8:8181/absent/search', { params: _this.ruleForm }).then(function (resp) {
        ...
        page(currentPage) {
            ...
            if (_this.ruleForm.value == '') {
                axios.get('http://172.20.10.8:8181/absent/list/' + currentPage + '/' + _this.pageSize).then(function (resp) {
                    _this.tableData = resp.data.data.data
                    _this.total = resp.data.data.total
                })
            } else {
    ...
    created() {
        const _this = this
        axios.get('http://172.20.10.8:8181/absent/list/1/' + _this.pageSize).then(function (resp) {
            _this.tableData = resp.data.data.data
            _this.total = resp.data.data.total
        })
    }
