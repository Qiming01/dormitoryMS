<template>
    <div style="margin-top: 60px;margin-left:330px;width: 300px;height: 500px;border: 0px solid red;" >
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="保修日期" prop="createDate">
                <el-date-picker
                        style="width: 200px"
...
<script>
    export default {
        ...
        methods: {
            buildingChange(){
                const _this = this
                axios.get('http://172.20.10.8:8181/fix/findDormitoryByBuildingId/'+_this.ruleForm.buildingId).then(function (resp) {
            ...
            dormitoryChange(){
                const _this = this
                axios.get('http://172.20.10.8:8181/fix/findStudentByDormitoryId/'+_this.ruleForm.dormitoryId).then(function (resp) {
            ...
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.post('http://172.20.10.8:8181/fix/save',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
                                _this.$alert('报修登记成功', '', {
                                    confirmButtonText: '确定',
                                    callback: action => {
                                        _this.$router.push('/FixRecord2')
                                    }
                                });
                            }
                        })
        ...
        created() {
            const _this = this
            axios.get('http://172.20.10.8:8181/fix/buildingList').then(function (resp) {
