<template>
    ...
    padding:20px;">
        ...
        <el-table :data="tableData" border stripe style="width: 100%;position: relative;top:-30px">
            <el-table-column header-align="center" align="center" fixed prop="id" label="ID" width="40%">
            </el-table-column>
...
export default {
    ...
    methods: {
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    axios.get('http://172.20.10.8:8181/student/search', { params: _this.ruleForm }).then(function (resp) {
        ...
        page(currentPage) {
            ...
            if (_this.ruleForm.value == '') {
                axios.get('http://172.20.10.8:8181/student/list/' + currentPage + '/' + _this.pageSize).then(function (resp) {
                    _this.tableData = resp.data.data.data
                    _this.total = resp.data.data.total
                })
            } else {
        ...
        del(row) {
            ...
            this.$confirm('确认删除【' + row.name + '】吗？', '提示', {
                ...
                axios.delete('http://172.20.10.8:8181/student/deleteById/' + row.id).then(function (resp) {
                    if (resp.data.code == 0) {
    ...
    created() {
        const _this = this
        axios.get('http://172.20.10.8:8181/student/list/1/' + _this.pageSize).then(function (resp) {
            _this.tableData = resp.data.data.data
            _this.total = resp.data.data.total
        })
    }
